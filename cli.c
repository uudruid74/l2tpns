...
int cli_arg_help(struct cli_def *cli, int cr_ok, const char *entry, ...)
{
	va_list ap;
	char *desc;
	char buf[16];
...
static int cmd_show_counters(struct cli_def *cli, const char *command, char **argv, int argc)
{
	...
	cli_print(cli, "%-30s%u", "open_sessions",		count_current_sessions());
#endif /* STAT_CALLS */
...
static const char * config_var_type(int configIndex) 
{
	switch (config_values[configIndex].type)
	{
	case STRING:
		return "STRING";
	case INT:
...
static const char * config_var_type_byKey(char *arg) 
{
	int i;
	for (i = 0; config_values[i].key; i++)
		if (!strncmp(config_values[i].key, arg, strlen(arg)))
		{
			return config_var_type(i);
		}
	return "UNKNOWN";
}

static const char * config_var_value(int configIndex)
{
	void *value = ((void *) config) + config_values[configIndex].offset;
	switch (config_values[configIndex].type)
	{
		case STRING:
		{
			size_t len = config_values[configIndex].size;
			void * dest = malloc (len + 1);
			strncpy(dest, (char *)value, len);
			return dest;
		}
		case INT:
		{
			void * dest = malloc (128);
			snprintf (dest,127,"%d",*(int *)value);
			return dest;
		}
		case UNSIGNED_LONG:
		{
			void * dest = malloc (128);
			snprintf (dest,127,"%ul",*(int *)value);
			return dest;
		}
		case SHORT:
		{
		...
		case IPv4:
		{
			void * dest = malloc (INET_ADDRSTRLEN);
			return inet_ntop(AF_INET,value,dest,INET_ADDRSTRLEN);
		}
		case IPv6:
		{
			void * dest = malloc(INET6_ADDRSTRLEN);
			return inet_ntop(AF_INET6,value,dest,INET6_ADDRSTRLEN);
		}
		case BOOL:
		{
			void * dest = malloc(6);
			if (*(int *)value)
				return strcpy (dest,"true");
			else
				return strcpy (dest,"false");
		}
		default:
			return NULL;
	}
...
static const char * config_var_value_byKey(char *arg) 
{
	int i;
	for (i = 0; config_values[i].key; i++)
		if (!strncmp(config_values[i].key, arg, strlen(arg)))
		{
			return config_var_value(i);
		}
	return "UNKNOWN";
}
...
static int cmd_set(struct cli_def *cli, const char *command, char **argv, int argc)
{
	...
	if (CLI_HELP_REQUESTED)
	{
		...
		case 1:
			{
				...
				for (i = 0; config_values[i].key; i++)
					...
					{
						const char *freeme = config_var_value(i);
						cli_error(cli, "  %-25s : %s", config_values[i].key, freeme);
						free ((void *)freeme);
					}
			}
			return CLI_OK;
		...
		case 2:
			{
				const char * freeme = config_var_value_byKey(argv[0]);
				cli_error (cli, "\t%s Value for variable (currently %s)",
					config_var_type_byKey(argv[0]), freeme);
				free((void *)freeme);
			}
			return CLI_OK;
